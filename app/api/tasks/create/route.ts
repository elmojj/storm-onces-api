    ...
    try {
      ...
      if (auth) {
        ...
        if (!retryResp.ok) {
          ...
          console.error("[create-task] 重试后仍然失败", retryResp.status, text);
          return NextResponse.json(
            {
              success: false,
 ...
 *
 * 使用示例：
 * GET /api/tasks/create?title=任务标题&userId=Ai5bfAgB&description=任务描述
 */
export async function GET(req: NextRequest) {
  try {
    ...
    return await createTask(title || "", userId || "", description);
  } catch (err: any) {
    console.error("/api/tasks/create GET error", err);
    return NextResponse.json(
      {
        success: false,
